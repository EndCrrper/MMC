...
本文针对2020年全国大学生数学建模竞赛A题，建立了一维非稳态热传导模型，对回焊炉焊接区域的温度变化进行数值模拟与参数优化。
...
求得最大传送带速度为$\boldsymbol{80.84\;\mathrm{cm/min}}$，限制瓶颈为峰值温度不低于$240^{\circ}\mathrm{C}$的约束。
...
面积从$392.5\;^{\circ}\mathrm{C}\!\cdot\!\mathrm{s}$增加至$409.1\;^{\circ}\mathrm{C}\!\cdot\!\mathrm{s}$，以$4.2\%$的面积代价换取了$1.6\%$的对称性改善。
...
\caption{炉内温度分布各段分界点}
\label{tab:oven}
\begin{tabularx}{\textwidth}{c@{\hspace{0.8cm}}>{\centering\arraybackslash}p{4cm}Y}
\toprule
...
\begin{figure}[H]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.1\textwidth]{result/figures/q1_furnace_curve.png}}
...
\boxed{v_{\max} = 80.84\;\mathrm{cm/min}}
\end{equation}
...
\begin{figure}[H]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.1\textwidth, keepaspectratio]{result/figures/q2_speed_analysis.png}}
...
\begin{figure}[H]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.1\textwidth]{result/figures/q3_optimal.png}}
...
\begin{figure}[H]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.1\textwidth]{result/figures/pso_convergence.png}}
...
\midrule
Q3 (最小面积)    & 392.5 & 0.1833 & ---  & ---  & 180.5 & 187.6 & 227.7 & 265.0 \\
Q4 (最小$\sigma$) & 409.1 & \textbf{0.1803} & 0.0711 & 0.1803 & 179.4 & 198.6 & 226.5 & 265.0 \\
\bottomrule
\end{tabularx}
\end{table}

问题4以面积增加$16.6\;^{\circ}\mathrm{C}\!\cdot\!\mathrm{s}$（约$4.2\%$）的代价，将对称性指标从$0.1833$改善至$0.1803$。
面积对称性$\sigma_1=0.0711$表明峰值两侧面积已高度均衡；时间对称性$\sigma_2=0.1803$是主要的不对称来源。

\begin{figure}[H]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.1\textwidth]{result/figures/q4_symmetric_optimal.png}}
...
面积对称性$\sigma_1=0.0711$已接近完全对称，但时间对称性$\sigma_2=0.1803$改善空间有限。
...
追求\textbf{最高产能}时，采用问题2方案（$v=80.84\;\mathrm{cm/min}$），在保证质量的前提下最大化生产速度；
...
\textbf{问题2}：通过二分搜索确定最大传送带速度为$80.84\;\mathrm{cm/min}$，限制瓶颈为峰值温度不低于$240^{\circ}\mathrm{C}$。
