...
生鲜蔬菜保质期短、品相随售卖时间衰减，商超需根据历史销售和市场需求制定每日补货与定价策略。本文围绕某商超6类蔬菜2020--2023年共878,504条销售流水、55,983条批发价格记录及损耗率数据，通过统计分析与可视化、时间序列预测、弹性需求定价和组合优化方法，系统解决了品类分布规律、销量预测与弹性定价、选品补货计划及数据采集建议四个递进问题。

针对\textbf{问题1}，将交易级流水聚合为46,599条日度记录，从品类分布、单品集中度、时间效应和关联关系四个维度进行了系统分析。结果表明：花叶类和辣椒类为销量最大的两个品类（合计占比49.8\%）；单品销量呈高度集中分布，Top-20单品贡献57.2\%的销量，Top-50达84.4\%，呈现出显著的Pareto特征；周末（周六/日）日均销量（12.6/12.3 kg/单品）约为工作日（8.7--9.8 kg）的1.4倍；品类间存在中等程度正相关（$r=0.07$--$0.69$），食用菌与辣椒类的相关性最高（$r=0.687$），反映了客流的共同驱动效应。基于对数--线性需求模型估计了各品类的价格弹性，范围从$-1.242$（水生根茎类）到$-0.116$（花菜类），所有弹性值均显著为负（$p<0.05$），验证了需求定律。

针对\textbf{问题2}，建立了"预测--定价联合优化"框架：销量预测采用加权移动平均结合星期效应系数的方法，定价优化以弹性需求函数为核心、以品类周总利润最大化为目标。对2023年7月1--7日的预测显示，六大品类日均总销量约196.6 kg，在考虑损耗率（6类平均约11.2\%）后日均补货量约220.9 kg。最优定价下预计周总利润约10,854元。敏感性分析表明，当价格弹性估计存在$\pm 20\%$偏差时，最优利润波动在$8\%$以内，定价策略具有较好的鲁棒性。

针对\textbf{问题3}，在27--33单品、每单品$\ge 2.5$ kg补货的约束下，构建了融合日利润、销量稳定性（CV倒数）和品质因子（打折率倒数）的三维综合评分体系。从6月24--30日参考周的49个可售单品中，采用"分类保底+贪心择优"策略筛选出最优33个单品。方案覆盖全部6个品类（花叶类14、辣椒类8、水生根茎类4、食用菌3、茄类2、花菜类2），预计单日日利润约629.8元，各品类均有覆盖，保证了品类丰富度和顾客选购体验。
...
\node[rbox, below=0.35cm of m2] (r2) {周利润10,854元  日补货220.9kg};
\node[rbox, below=0.35cm of m3] (r3) {33单品  全覆盖  日利润630元};
...
\midrule
附件1 & 252 & 4 & -- & 单品编码、名称、分类编码、分类名称 \\
附件2 & 878,504 & 7 & 2020-07-01至2023-06-30 & 销售日期、扫码时间、单品、销量、单价、类型、是否打折 \\
附件3 & 55,983 & 3 & 同上 & 日期、单品编码、批发价格（元/kg） \\
附件4 & 252+6 & 2--3 & -- & 单品损耗率（\%）+ 品类平均损耗率（\%） \\
\bottomrule
\end{tabularx}
\end{table}

数据质量方面：附件2的销售流水覆盖了从2020年7月1日到2023年6月30日共1096个自然日，其中有销售记录的天数为1095天（仅1天无销售）；在售单品共246个，占附件1登记单品的97.6\%；单品日度记录共计46,599条。附件3的批发价格记录覆盖了同期大部分单品，以日期--单品为键匹配至销售记录的覆盖率为100\%。
...
由表可见，花叶类以89个单品（占总单品数的35.3\%）贡献了30.2\%的总销量，是绝对的核心品类。辣椒类和食用菌虽然单品数较少（分别为45和18个），但单品平均销量较高——食用菌以18个单品实现了17.0\%的占比，单品平均销量（25.5吨）约为花叶类（9.2吨/单品）的2.8倍，体现了高集中度特征。
...
\midrule
水生根茎类 & $-1.242$ & 0.021 & $-58.9$ & $<0.001$ & 0.186 \\
辣椒类 & $-0.977$ & 0.018 & $-54.3$ & $<0.001$ & 0.152 \\
花叶类 & $-0.864$ & 0.015 & $-57.6$ & $<0.001$ & 0.165 \\
茄类 & $-0.730$ & 0.022 & $-33.2$ & $<0.001$ & 0.091 \\
食用菌 & $-0.399$ & 0.019 & $-21.0$ & $<0.001$ & 0.048 \\
花菜类 & $-0.116$ & 0.026 & $-4.5$ & $0.041$ & 0.008 \\
\bottomrule
\end{tabularx}
\end{table}

弹性的经济学含义解读如下：水生根茎类弹性最大（$-1.242$），意味着涨价1\%将导致需求量下降约1.24\%，属于富有弹性的商品，降价促销对该品类效果最为显著；食用菌（$-0.399$）和花菜类（$-0.116$）弹性较小，属于缺乏弹性的必需品，涨价不会显著减少销量——对于这两类商品，适度的溢价策略可以提升总收益。

\textbf{模型检验。}为评估弹性估计的稳健性，进行了两项补充检验：（1）使用滚动窗口（90天和180天）估计弹性，观察参数的时变稳定性；（2）使用Bootstrap重采样（200次）计算弹性估计的置信区间。结果显示各品类弹性估计在时间窗口下保持方向一致，Bootstrap 95\%置信区间不含零，表明弹性估计具有统计稳健性。
...
\begin{tabularx}{\textwidth}{lYYYYY}
\toprule
品类 & 近4周日均(kg) & 趋势(kg/年) & 星期系数范围 & 损耗率 \\
\midrule
花叶类 & 55.2 & $-12.3$ & 0.81--1.22 & 10.0\% \\
辣椒类 & 41.8 & $+5.7$ & 0.76--1.31 & 10.5\% \\
食用菌 & 38.5 & $-3.2$ & 0.82--1.28 & 10.0\% \\
水生根茎类 & 28.4 & $+2.1$ & 0.79--1.27 & 12.8\% \\
茄类 & 24.2 & $-1.8$ & 0.80--1.25 & 10.0\% \\
花菜类 & 16.5 & $-4.5$ & 0.72--1.35 & 15.5\% \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[H]
\centering
\caption{7月1--7日各品类日销量预测（kg）}
\label{tab:daily_pred}
\begin{tabularx}{\textwidth}{lYYYYYYYY}
\toprule
品类 & 7/1(六) & 7/2(日) & 7/3(一) & 7/4(二) & 7/5(三) & 7/6(四) & 7/7(五) \\
\midrule
花叶类 & 61.9 & 58.3 & 40.3 & 37.8 & 41.5 & 40.5 & 47.6 \\
辣椒类 & 46.8 & 43.6 & 30.2 & 28.3 & 31.0 & 30.2 & 35.7 \\
食用菌 & 42.1 & 39.5 & 27.8 & 26.1 & 28.6 & 27.9 & 32.6 \\
水生根茎类 & 28.8 & 27.0 & 18.9 & 17.8 & 19.4 & 18.9 & 22.2 \\
茄类 & 24.0 & 22.5 & 15.6 & 14.6 & 16.0 & 15.6 & 18.3 \\
花菜类 & 15.6 & 14.6 & 10.1 & 9.5 & 10.4 & 10.2 & 11.9 \\
\midrule
合计 & 219.2 & 205.5 & 142.9 & 134.1 & 146.9 & 143.3 & 168.3 \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
花叶类 & 5.62 & 47\% & 8.42 & 50\% & 74.5 & 2,652.7 \\
辣椒类 & 6.83 & 52\% & 10.24 & 50\% & 56.4 & 2,193.5 \\
食用菌 & 7.90 & 38\% & 11.85 & 50\% & 52.0 & 2,159.1 \\
水生根茎类 & 5.18 & 55\% & 7.77 & 50\% & 38.4 & 1,631.9 \\
茄类 & 5.30 & 48\% & 7.95 & 50\% & 32.7 & 1,252.5 \\
花菜类 & 4.76 & 42\% & 7.14 & 50\% & 26.3 & 964.1 \\
\midrule
合计 & -- & -- & -- & -- & 280.3 & 10,853.8 \\
\bottomrule
\end{tabularx}
...
\textbf{定价结果解读。}各品类的最优加价率集中于45\%--55\%区间。弹性较大的品类（水生根茎类）和弹性较小的品类（花菜类、食用菌）的最优加价率差异被弹性需求函数的自我调节机制所调和：弹性大的品类提价会显著降低销量从而抑制利润增长，弹性小的品类提价对销量影响较小从而鼓励更高加价。最终各品类的均衡加价率趋于相近。

\textbf{敏感性分析。}为评估最优定价策略对弹性估计误差的鲁棒性，进行了弹性参数敏感性测试：对每个品类的弹性系数施加$\pm 10\%$和$\pm 20\%$的扰动，重新求解最优价格和对应利润，结果如表\ref{tab:elastic_sens}所示。

\begin{table}[H]
\centering
\caption{弹性参数敏感性分析（周利润变化率）}
\label{tab:elastic_sens}
\begin{tabularx}{\textwidth}{lYYYY}
\toprule
品类 & $\eta-20\%$ & $\eta-10\%$ & $\eta+10\%$ & $\eta+20\%$ \\
\midrule
花叶类 & $+4.2\%$ & $+2.0\%$ & $-1.8\%$ & $-3.5\%$ \\
辣椒类 & $+5.1\%$ & $+2.4\%$ & $-2.2\%$ & $-4.1\%$ \\
食用菌 & $+2.8\%$ & $+1.3\%$ & $-1.2\%$ & $-2.3\%$ \\
...
结果表明：当弹性估计存在$\pm 20\%$的误差时，最优利润的波动幅度在$8\%$以内，定价策略具有较好的鲁棒性。这一结论为商超在实际中应用弹性定价提供了信心——即使弹性估计不够精确，基于近似弹性制定的差异化定价策略仍能有效提升利润。
...
\midrule
花叶类 & 14 & 148.3 & 150.2 & 262.1 \\
辣椒类 & 8 & 87.2 & 88.6 & 156.4 \\
水生根茎类 & 4 & 35.4 & 36.1 & 78.3 \\
食用菌 & 3 & 38.7 & 39.5 & 65.8 \\
茄类 & 2 & 21.5 & 21.8 & 38.6 \\
花菜类 & 2 & 14.8 & 15.3 & 28.6 \\
\midrule
合计 & 33 & 345.9 & 351.5 & 629.8 \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{方案特征分析。}（1）品类分布与销量占比高度一致——花叶类和辣椒类分别入选14和8个单品，合计占入选总数的66.7\%，与其在总销量中49.8\%的占比相匹配。（2）食用菌虽然单品数少（3个），但单品平均日销量（12.9 kg）和日利润（21.9元）均为最高，体现了"少而精"的特征，与其高单品平均销量的历史规律一致。（3）所有6个品类均至少入选2个单品，保证了品类丰富度，为顾客提供了完整的选购体验，避免了因品类缺失导致的顾客流失。

\textbf{方案对比。}对$N$从27到33遍历搜索的结果如表\ref{tab:n_comp}所示。在27--33范围内，总利润随$N$增加而递增，在$N=33$时达到最优。利润递增呈边际递减趋势——从27增至30时利润增长明显（每增加1个单品约增加8--12元），从30增至33时增速放缓（每增加1个单品约增加4--6元），表明排名靠后的单品边际贡献较低。
...
\midrule
总日利润(元) & 578.3 & 588.7 & 598.1 & 607.0 & 615.2 & 622.8 & 629.8 \\
边际利润(元) & -- & 10.4 & 9.4 & 8.9 & 8.2 & 7.6 & 7.0 \\
\bottomrule
\end{tabularx}
\end{table}
...
问题2的弹性定价模型得出了一个看似反直觉的结果：各品类的最优加价率趋于相近（45\%--55\%），而非弹性大的品类低加价、弹性小的品类高加价。这一现象可以从利润最大化的数学结构中理解。

对利润函数$\Pi_i(P_i)$求一阶条件$\partial \Pi_i / \partial P_i = 0$，整理可得最优加价率的解析表达式：
\begin{equation}
    M_i^* = \frac{-\eta_i}{1 + \eta_i} \cdot \frac{W_i / (1-L_i)}{P_i^0} - 1
\end{equation}
该式表明最优加价率取决于弹性$\eta_i$、批发价$W_i$、损耗率$L_i$和当前价格水平$P_i^0$的联动关系。由于各品类的批发价和损耗率存在差异，它们与弹性的综合效应使得最优加价率趋于相近，而非简单按弹性排序。
...
(1) 蔬菜销售呈现高度集中的Pareto分布（Top-50单品贡献84.4\%销量），具有稳定的星期周期模式（周末销量约为工作日的1.4倍），品类间正相关主要由客流共同驱动（$r=0.07$--$0.69$）。各品类价格弹性为$-1.24$至$-0.12$，均显著为负，验证了需求定律。

(2) 基于弹性需求函数和移动平均预测的联合优化框架，能够为商超提供品类级的最优定价和日补货量建议。预测期（7月1--7日）预计日均总销量约196.6 kg，日均补货量约220.9 kg，在最优定价下的预计周总利润约10,854元。定价策略对弹性估计误差具有良好的鲁棒性（$\pm 20\%$误差下利润波动$\le 8\%$）。

(3) 在27--33单品的陈列约束下，通过"分类保底+贪心择优"策略可选出覆盖全部6个品类的33个最优单品，预计单日日利润约629.8元。品类平衡约束虽使理论利润略微下降（约2\%--3\%），但从顾客体验角度具有重要的长期战略价值。
